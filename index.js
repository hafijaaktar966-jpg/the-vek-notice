...
// API Endpoint for generating code with OpenAI
app.post('/generate-code', async (req, res) => {
    try {
        const { prompt, language } = req.body;

        if (!process.env.OPENAI_API_KEY) {
            return res.status(500).json({ error: 'OPENAI_API_KEY missing in Render Environment Variables!' });
        }

        const completion = await openai.chat.completions.create({
            model: 'gpt-4o-mini',
            messages: [
                { role: 'system', content: `You are an expert coding assistant. Write clean, efficient, and well-commented code in ${language}. Only return the code block without markdown formatting or conversational text.` },
...
// Bind to 0.0.0.0 for Render hosting compatibility
